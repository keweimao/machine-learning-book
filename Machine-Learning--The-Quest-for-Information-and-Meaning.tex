...
Figure~\hyperref[fig-data-plot1]{1.1}.
...
well, we get Figure~\hyperref[fig-data-plot2]{1.2}.
...
\item
  ...
  \item
    ...
    \item
      ...
      Scatter plots with color-coded {V4}. Mark {∘} for {0} and {+} for
      {1} values.
    \end{enumerate}
  \end{enumerate}
\end{enumerate}
...
values in Figure~\hyperref[fig-data-plot2]{1.2} (b), showing the scatter
plot of the \(3^{rd}\) vs.~\(2^{nd}\) column. When the value of the
\(3^{rd}\) column is greater than the value of the \(2^{nd}\), it is
\(1\) in the \(4^{th}\) column; otherwise, it gives \(0\). This can be
written as:
...
Equation~\hyperref[eq-data-ne]{{[}eq-data-ne{]}} is a rule-based
approach to concept representation (descriptor). Sometimes, a (linear)
equation may also be used to represent the result of learning. In this
example, equation \(V_3 - V_2 = 0\) is the straight line that separates
the \(0\) and \(1\) values of column \(4\), as shown in
Figure~\hyperref[fig-data-plot3]{{[}fig-data-plot3{]}}.

Whether the concept representation is based on the rules in
Equation~\hyperref[eq-data-ne]{{[}eq-data-ne{]}} or the linear equation
in Figure~\hyperref[fig-data-plot3]{{[}fig-data-plot3{]}}, it can be
identified by computing and comparing values in the data. Normally such
concept representation is discovered through \emph{training data} and to
be applied on testing data, where decisions or predictions will be made.
...
in Figure~\hyperref[fig-data-shapes]{{[}fig-data-shapes{]}}.
Intuitively, the \emph{concept} of standing can be understood as
describing a human-like object -- when a person stands, her height is
greater than the width; when one lies down, the horizontal measurement
(width) becomes relatively greater.
...
Figure~\hyperref[fig-data-building]{{[}fig-data-building{]}}, for
example, the shape of a building may be flat in terms of its
height-width ratio but we see it \emph{standing as an architecture}.
...
which is illustrated by the shaded area in
Figure~\hyperref[fig-num-overlap]{{[}fig-num-overlap{]}}. The
interaction operation is communicative, i.e.~\(A \cap B=B \cap A\),
...
for any sets A, B, and C. As illustrated in Figure
\hyperref[fig-num-union3]{{[}fig-num-union3{]}}, we will obtain the same
union set (the shaded circles) regardless of which two are combined
first. The same is true with intersection, as illustrated in
Figure~\hyperref[fig-num-overlap3]{{[}fig-num-overlap3{]}}.
...
(see Figure~\hyperref[fig-num-sub]{{[}fig-num-sub{]}}), their
intersection is A and their union is B:
...
Figure~\hyperref[fig-num-minus]{{[}fig-num-minus{]}}.
...
Figure~\hyperref[fig-num-vectors]{{[}fig-num-vectors{]}} shows data
\(v_1\) and \(v_2\) in a space with \(X\) and \(Y\) dimensions. In the
vector space, each data point is a vector from the origin \((0,0)\), as
shown as directed lines in
Figure~\hyperref[fig-num-vectors]{{[}fig-num-vectors{]}}, and can be
represented by its dimensional values:
...
Figure~\hyperref[fig-matrix-x]{{[}fig-matrix-x{]}}.

Let \(Y\) be this year's data as follow, which, likewise, can be
visualized as \(3\) row vectors in
Figure~\hyperref[fig-matrix-y]{{[}fig-matrix-y{]}}:
...
\(g_3\). As shown Figure~\hyperref[fig-matrix-xy]{{[}fig-matrix-xy{]}},
subtraction \(g_3=y_3-x_3\) is the vector that starts at \(x_3\) and
ends at \(y_3\).
...
or column -- in the matrix. For example,
Figure~\hyperref[fig-matrix-2g]{{[}fig-matrix-2g{]}} shows when \(g_3\),
the growth vector of the \(3^{rd}\) company (row), multiplies \(2\), the
vector doubles its length (magnitude) in the same direction. Multiplying
a matrix with a scalar (single number) results in a matrix of the same
dimensionality, in which every value is multiplied by the same scalar.
Note the multiplication can be written as \(2 \cdot G\), \(2G\) or, more
generally, \(aG\) given a scalar \(a\) and a matrix \(G\).
...
solid lines in Figure~\hyperref[fig-matrix-z]{{[}fig-matrix-z{]}}, for
the \(3^{rd}\) row vector, \(x_3+g_3\times2\) is to connect vectors
\(x_3\) and \(2g_3\) and the result is \(z_3\), which draws from the
tail of \(x_3\) to the head of \(2g_3\). Vector and matrix addition is
communicative, the same result can be obtained by \(2g_3 + x_3\), as
shown by dashed lines in
Figure~\hyperref[fig-matrix-z]{{[}fig-matrix-z{]}}.
...
which is the length shown in
Figure~\hyperref[fig-matrix-magnitude]{{[}fig-matrix-magnitude{]}}.
...
Figure~\hyperref[fig-matrix-distance]{{[}fig-matrix-distance{]}}.
...
As shown in Figure~\hyperref[fig-matrix-unit]{{[}fig-matrix-unit{]}},
the normalized unit vectors \(\hat{u}\) and \(\hat{v}\) point to the
same directions of their original vectors \(u\) and \(v\) respectively.
However, the normalization produce vectors of equal magnitudes. As the
dotted circle shows, the unit vectors are part of a unit circle with
radius \(1\).
...
\protect\phantomsection\label{fig-matrix-cosine}{}

which is the cosine of the angle between \(u\) and \(v\), shown in
Figure~\hyperref[fig-matrix-unit]{{[}fig-matrix-unit{]}}. The arc cosine
of \(0.894\) corresponds to an angle of roughly \(27^\circ\) out of a
\(360^\circ\) full circle, or \(0.465\) on a \(2\pi\) scale. Cosine
values range between \(-1\) and \(1\): it is \(1\) when two vector are
in the exact same direction (parallel) and \(-1\) when they are in the
opposite direction (a \(180^\circ\) or \(\pi\) angle). Cosine is \(0\)
when two vectors are orthogonal (perpendicular) to each other (right
angle). See
Figure~\hyperref[fig-matrix-cosine]{{[}fig-matrix-cosine{]}}.
...
\bottomrule\noalign{}
\endlastfoot
\pandocbounded{\includegraphics[keepaspectratio,alt={image}]{chapters/../assets/figures/prob/coin_head.png}}\{w
& idth=``1in''\}
\includegraphics[width=1in,height=\textheight,keepaspectratio,alt={image}]{chapters/../assets/figures/prob/coin_tail.png} \\
\end{longtable}
}
...
{\centering \includegraphics[width=2in,height=\textheight,keepaspectratio]{chapters/../assets/figures/prob/p_no_overlap.png}
...
Figure~\hyperref[fig-prob-no_overlap]{{[}fig-prob-no\_overlap{]}}, have
\emph{no overlap} or intersection in their probability space. Because of
that, we can simply add the two together.
...
{\centering \includegraphics[width=2in,height=\textheight,keepaspectratio]{chapters/../assets/figures/prob/p_overlap.png}
...
illustrated in
Figure~\hyperref[fig-prob-overlap]{{[}fig-prob-overlap{]}}. As shown in
the figure, when A and B can occur simultaneously, they have an overlap
...
{\centering \includegraphics[width=2in,height=\textheight,keepaspectratio]{chapters/../assets/figures/prob/ace_spades.png}
...
elephant} shows (Figure~\hyperref[fig-prob-elephant]{1.1}). But we
should aim to see the whole picture.
...
Frequency distribution of flipping a coin, 500 heads vs.~500 tails after
{1, 000} trials in the ideal world.
...
Probability distribution of flipping a coin (500 heads vs. 500 tails).
...
distribution, as Figure~\hyperref[fig-prob-coin_prob]{1.3} shows.
...
Figure~\hyperref[fig-prob-dice]{1.4}.

Probability distribution of rolling a dice
...
Figure~\hyperref[fig-prob-cont]{1.5}.

Probability distribution of a continuous variable {x}
...
Figure~\hyperref[fig-prob-cont]{1.5} and it is easy to calculate the
area. In the above case, the area of the rectangle between \(1\) and
\(6\) can be computed by width times height, which is
\((6-1) \times 0.2 = 1\). So this meets the expectation that the sum of
the entire probability distribution is \(1\).
...
Figure~\hyperref[fig-prob-cont]{1.5} is NOT a probability value per se.
It is what we refer to as a \emph{probability density}, but not exactly
a probability. For now, let us move on from the uniform distribution to
a more common distribution in the real world before we come back to
...
Distribution of adult male heights, approximately a normal distribution
(bell curve)
...
distribution presented in Figure~\hyperref[fig-prob-cont]{1.5}, the
\(0.2\) value on y is not probability but probability density or
probability per unit. You can still tell that all probabilities are
equal by looking at the flat line. And you can ask the question about
how likely you will get a value between 2 and 4 by multiplying the
density which is \(0.2\) and the range which is \(2\) and the result
turns out to be \(0.4\) -- that is \(40\%\) of the chance you will get a
value \emph{in that range}.
...
Figure~\hyperref[fig-prob-cont]{1.5}, the answer is that it will
\emph{always be true}.
...
Figure~\hyperref[fig-prob-word_freq]{1.7} shows the distribution of word
frequencies taken from the novel \emph{Moby Dick}. As shown in
Figure~\hyperref[fig-prob-word_freq]{1.7} (a), there is a great divide
between the rich and the poor -- a large number of words have only been
used once or twice in the novel (tall bars on the left of the
distribution) where very few words have a frequency higher than \(100\)
(the long tail).
...
The result is Figure~\hyperref[fig-prob-word_freq]{1.7} (b), where both
\(x\) (words' occurrences) and \(y\) (frequencies of the occurrences)
have been transformed with logarithm.\footnote{Essentially
  log-transformation reduces a number to its order, e.g. \(1000\) to
  \(3\) and \(100\) to \(2\) with \(log_{10}\).}
...
Figure~\hyperref[fig-info-h]{{[}fig-info-h{]}} shows that the entropy
function is smooth with regard to changes in the probability \(p_1\).
When the two mutually exclusive events are equally likely
\(p_1=p_2=0.5\), the entropy of the system is at its maximum (the peak
in the middle).
...
Figure~\hyperref[fig-info-h_m]{{[}fig-info-h\_m{]}} plots entropy \(H\)
vs.~the number of equally likely events \(m\). When the number of events
(choices) increases, so does the entropy. With an increasingly larger
pool of applicants for a position -- supposedly with equal chances --
the harder it is to predict who will be hired in the end.
...
Figure~\hyperref[fig-info-triangle]{{[}fig-info-triangle{]}}, where any
two distances (sides) \(d(A,B) + d(B,C) \ge d(A,C)\).
...
Figure~\hyperref[fig-info-lit]{1.1} compares least information (LIT)
with Shannon entropy and KL divergence, using a binary example (of two
mutually exclusive inferences). The figure assumes the \(1^{st}\)
inference to be the ultimate outcome -- that is, it starts with a
probability distribution \(P\) (with some \(p_1\) and \(p_2\)) and ends
with changed distribution \(Q\), where \(q_1=1\) and \(q_2=0\). For
example, the reading of \((0.5, 1)\) on the LIT curve indicates the
amount of least information is \(1\) that changes the probability
distribution from \(P(0.5, 0.5)\) to \(Q(1, 0)\).
...
\item
  ...
  \((\frac{1}{2},1)\) in Figure~\hyperref[fig-info-lit]{1.1}).
\item
...
illustrated in
Figure~\hyperref[fig-info-dist_add]{{[}fig-info-dist\_add{]}}, where
\(a+b=c\). This is the only situation in which they are equal with the
triangle inequality.
...
data instance. Figure~\hyperref[fig-bin-ed]{{[}fig-bin-ed{]}} shows the
shapes data on two dimensions, i.e. \emph{width} and \emph{height}.
Suppose a new instance at \((4,7)\), i.e.~\(width=4\) and \(height=7\),
is to be tested. The dashed lines are the euclidean distances from the
test instance to the training instances.
...
Figure~\hyperref[fig-bin-ed2]{{[}fig-bin-ed2{]}} shows another test
instance at \((3,4)\). With \(k=3\), it turns out that the three nearest
neighbors include \(1\) positive and \(2\) negatives. Using a majority
vote strategy, the test instance will be mistakenly classified into the
negative (lying) class.
...
as illustrated in Figure~\hyperref[fig-bin-ed2]{{[}fig-bin-ed2{]}}.
Finding the optimal \(k\) may require experimentation with data.
...
With the shapes data in Figure~\hyperref[fig-bin-ed]{{[}fig-bin-ed{]}},
there are two variables \(width\) (\(v_1\) for the \(x\) axis) and
\(height\) (\(v_2\) for \(y\) axis). \(y\) as a linear function of \(x\)
can be written as \(f(x) = a + b x\) or \(a + b x - y = 0\), with two
...
Figure~\hyperref[fig-bin-lin_cent]{{[}fig-bin-lin\_cent{]}} shows the
two centroids and a dashed line with an arrowhead connecting them. The
arrow represents a vector in the direction
...
Figure~\hyperref[fig-bin-lin_cent]{{[}fig-bin-lin\_cent{]}}.
...
\((4,7)\) (\(\otimes\) on
Figure~\hyperref[fig-bin-lin_cent]{{[}fig-bin-lin\_cent{]}}), we can put
the values in the linear function and get:
...
Figure~\hyperref[fig-bin-lin_cent2]{{[}fig-bin-lin\_cent2{]}} (compare
to Figure~\hyperref[fig-bin-lin_cent]{{[}fig-bin-lin\_cent{]}}).
...
Figure~\hyperref[fig-bin-svm]{{[}fig-bin-svm{]}} illustrates the idea
with the shapes data. \footnote{Please note additional data points than
  previously discussed in the illustration.} Pairwise lines drawn in the
figure all satisfy the \(1^{st}\) condition and collectively form a
\emph{convex hull} for each class. However, not all lines meet the
\(2^{nd}\) condition. In fact, only the lines marked \(1\), \(2\), and
\(3\) meet both conditions. They are candidate \emph{support vectors}.
...
example, in Figure~\hyperref[fig-bin-svm]{{[}fig-bin-svm{]}}, the two
dotted lines show the margin between line \(\#3\) and the negative
class.
...
the two dotted lines in
Figure~\hyperref[fig-bin-svm2]{{[}fig-bin-svm2{]}} show. The greatest
...
data points in Figure~\hyperref[fig-bin-svm2]{{[}fig-bin-svm2{]}},
support vectors are those that determine the maximum margin between the
two \emph{convex hulls} of the two classes.

With this, it is now easy to identify the linear equation in the margin
to separate the two classes. In
Figure~\hyperref[fig-bin-svm2]{{[}fig-bin-svm2{]}}, for example, the
solid line at \(v_2 = 1.1 + 0.667 v_1\) can be used as the linear
...
as a \emph{perceptron} in
Figure~\hyperref[fig-bin-perceptron]{{[}fig-bin-perceptron{]}}, where
the node at \(\sum\) computes the weighted sum of input variables.
...
Figure~\hyperref[fig-bin-perceptron]{{[}fig-bin-perceptron{]}}. The
final output of the perceptron is binary \(0\) or \(1\) by using a step
function, which simply converts values higher than a threshold to \(1\)
and those lower to \(0\). For example, one can use \(0\) as the
threshold for a step function \(f(s)\):
...
Figure~\hyperref[fig-bin-step]{{[}fig-bin-step{]}}.
...
Note that in
Figure~\hyperref[fig-bin-perceptron]{{[}fig-bin-perceptron{]}}, the
input with weight \(w_0\) is always \(1\) for every training instance.
This is referred to as the \emph{bias}, which adds the constant \(w_0\)
to the linear function. Once every input is multiplied by its weight,
the output computes the sum of the weighted values.
...
discussion here. Figure~\hyperref[fig-bin-car]{{[}fig-bin-car{]}} plots
\emph{maintenance cost} vs.~\emph{buying price}, with the \(+\)
indicating an \emph{acceptable} car and \(\circ\) unacceptable. The two
classes appear not separable linearly on the two-dimensional space --
that is, there is no straight line that can be drawn to separate the
two.
...
dashed line in Figure~\hyperref[fig-bin-car]{{[}fig-bin-car{]}}, a
circular function such as \((v_1-1)^2 + (v_2 -1)^2 = 3\) can set the
boundary between the two classes. The question is: how can we identify
such as a non-linear function from the training data?
...
Figure~\hyperref[fig-bin-car_q2]{{[}fig-bin-car\_q2{]}}, the solid line
at \(v_1+v_2-9=0\) can separate the data we have so far. However, this
does not necessarily represent the general idea of car evaluation and
new data may violate the rule (cross the decision boundary).
...
Figure~\hyperref[fig-bin-car_q3]{1.1}, where they may be separated
linearly. Of all linear equations (planes) formed by any three points in
each class, we have identified two planes closest to their opposite
classes.
...
\item
  ...
  \tightlist
  \item
    Zoom in on support vectors

    Car evaluation data, mapped to higher dimensions with {v12}, {v22},
    and {\(\sqrt{2} v_1 v_2\)}. The two planes are closest to the
    opposite class. The projected shadows are for reference only. (a)
    shows all data points in the three dimensional space, and (b) is a
    closer view of the two planes with support vectors. Dashed line is
    the gap (margin) between the two classes. Circled data points are
    support vectors.
  \end{enumerate}
\end{enumerate}

Figure~\hyperref[fig-bin-car_q3]{1.1} (a) shows all data points in the
expanded dimensional space with quadratic mapping. The two planes formed
by data points are the closest to the opposite class, which are
candidate support vectors. Shadows of the planes are projected to the
"ground" (i.e.~\(v_2^2\) vs.~\(v_1^2\) dimensions) for easier reading of
related data points.

Figure~\hyperref[fig-bin-car_q3]{1.1} (b) zooms in on the planes formed
by candidate support vectors in the three-dimensional space, where a
...
Figure~\hyperref[fig-bin-car]{{[}fig-bin-car{]}}, cannot be separated by
one linear function. However, with a step function, one can transform
the data into something linearly separable. Specifically, for a value
\(s\) on the \(v_1\) or \(v_2\) dimension, we use the following step
function:
...
Figure~\hyperref[fig-bin-car_step]{{[}fig-bin-car\_step{]}}. All
positive (acceptable) data have been mapped to the \((0,0)\) point
whereas the negative (unacceptable) ones are at \((0,1)\), \((1,1)\),
and \((1,0)\). As the solid line in
Figure~\hyperref[fig-bin-car_step]{{[}fig-bin-car\_step{]}} shows, the
transformed data are now linearly separable.
...
Figure~\hyperref[fig-bin-nn]{1.2}, each of the input variables \(v_1\)
and \(v_2\) is normalized by a step function and then multiplied by
\(-1\) before feeding into the final output, which is combined with the
bias weighted \(0.5\). This is equivalent to
\(0.5 \times 1 -1\times f(v_1) - 1\times f(v_2)\), where \(f\) is the
step function for \(v_1\) and \(v_2\) values.

Multi-layer neural network with step functions. {∑} represents the
weighted sum of input values leading to the present node.
...
Figure~\hyperref[fig-bin-nn]{1.2}.
...
Figure~\hyperref[fig-bin-step]{{[}fig-bin-step{]}}, has a sudden jump
(not smooth transition) between \(0\) and \(1\) and is not
differentiable.
...
for a value \(v\).
Figure~\hyperref[fig-bin-sigmoid]{{[}fig-bin-sigmoid{]}} shows the
functional curve of sigmoid, which transforms the weighted sum of a node
to a value approaching \(0\) or \(1\). The threshold is \(0\) by default
but we can use any threshold for the transformation. For example, with a
threshold of \(5\), the sigmoid function becomes:
...
(dashed line in
Figure~\hyperref[fig-bin-sigmoid]{{[}fig-bin-sigmoid{]}}).
...
Figure~\hyperref[fig-bin-nn]{1.2}, we have the multi-layer neural
network in Figure~\hyperref[fig-bin-nn_sigmoid]{1.3}, in which each
layer (node) is differentiable and the weight can be learned by gradient
descent.

Multi-layer neural network with the sigmoid function. {s1} and {s2} are
the weighted sum of input values leading to the respective hidden nodes
whereas {s} is the weighted sum of input values leading to the output
layer.
...
In Figure~\hyperref[fig-bin-nn_sigmoid]{1.3}, this can be written as:
...
which, for the model in Figure~\hyperref[fig-bin-nn_sigmoid]{1.3}, can
be computed by:
...
\(w_1\), and \(w_2\) weights in
Figure~\hyperref[fig-bin-nn_sigmoid]{1.3}.
...
Figure~\hyperref[fig-bin-nn_sigmoid]{1.3}. This approach of revising
weights to the two layers can thought of as propagating differentials of
squared errors \emph{backward} in the feed-forward neural network, and
is referred to as \emph{backpropagation}.
...
Figure~\hyperref[fig-bin-nn_sigmoid_0]{{[}fig-bin-nn\_sigmoid\_0{]}},
given the first training instance of \((2,2)\), we can compute:
...
Figure~\hyperref[fig-bin-nn_sigmoid_1]{{[}fig-bin-nn\_sigmoid\_1{]}}
shows the model with updated weights after training by the first
instance \((2,2)\) with a known class label \(1\). We can continue to
train the model with additional data instances in
Table~\hyperref[tab-bin-car_nn]{1.3}. This can be repeated with the same
data until weights are stabilized and the error is reduced to a certain
level.
...
\tightlist
\item
  Weight {\(\frac{w_{11}}{w_{10}}\)} to hidden layer
  ...
  \item
    ...
    \item
      ...
      Neural network training iterations and convergence
    \end{enumerate}
  \end{enumerate}
\end{enumerate}

Figure~\hyperref[fig-bin-car_nn_converge]{1.4} shows relative weights
and error over time, via iterations of repeated training with data in
...
their convergence. In Figure~\hyperref[fig-bin-car_nn_converge]{1.4}
(a), the relative weight of \(w_{11}\) (the weight of \(v_1\) to the
hidden node \(h_1\)) fluctuates a lot in the first \(10\) iterations but
stabilizes quickly after a few dozen iterations. In
Figure~\hyperref[fig-bin-car_nn_converge]{1.4} (b), \(w_1\) -- the
weight from the hidden node \(h_1\) to the final output -- appears to
take hundreds of iterations to converge. The squared error, as shown in
Figure~\hyperref[fig-bin-car_nn_converge]{1.4} (c), takes even even more
...
Figure~\hyperref[fig-bin-car_nn_result]{1.5}, which is highly similar to
the one in Figure~\hyperref[fig-bin-nn_sigmoid]{1.3}. For example, the
weighted sum for hidden node \(h_1\) is \(2.5 - v_1\) in the model here,
which is simply the negative of what is in
Figure~\hyperref[fig-bin-nn_sigmoid]{1.3}, where the sum is
\(v_1 - 2.5\). We observe the same with the hidden node \(h_2\). This
leads to the different signs of \(w_1, w_2\) vs.~\(w_0\) to the output
layer.

Neural network final model. Compare to Figure~1.3. Note that we
transform the bias node on the hidden layer with the sigmoid as well,
i.e.~{\(h_0 = f(1) = \frac{1}{1+e^{-1}} = 0.73\)}. In the final model,
{w0h0 = −1.92 × 0.73 = −1.4}.

Figure~\hyperref[fig-bin-car_nn_result_plot]{{[}fig-bin-car\_nn\_result\_plot{]}}
visualizes how the model transforms the original data into something
separable. A sigmoid function with a weighted sum of \(2.5 - v\) (for
both \(v_1\) and \(v_2\)) converts the original data (\(+\) and
\(\circ\) in faded colors) to values close to \(0\) (if \(v>2.5\)) and
\(1\) values (if \(v<2.5\)).
...
in Figure~\hyperref[fig-bin-car_nn_result]{1.5} which is different from
what we proposed in Figure~\hyperref[fig-bin-nn_sigmoid]{1.3}. Both
models will make perfect classifications on the training data presented
in Table~\hyperref[tab-bin-car_nn]{1.3}. However, a model thus obtained
has not necessarily found the global optimal. In fact, the
back-propagation mechanism via gradient descent can converge to a local
optimum, which depends on many factors such as the starting point
(initial values) and the sequence of training data. The learning rate
also impacts the convergence of weights and, occasionally, may also
influence where they converge.
...
Figure~\hyperref[fig-org-partitions]{{[}fig-org-partitions{]}} shows an
example of two flat partitions, where data belong to either one of them.
...
Figure~\hyperref[fig-org-hierarchy]{{[}fig-org-hierarchy{]}} illustrates
...
Figure~\hyperref[fig-org-hierarchy_circles]{{[}fig-org-hierarchy\_circles{]}}
can also be represented as a tree structure in
Figure~\hyperref[fig-org-hierarchy]{{[}fig-org-hierarchy{]}}. A
hierarchy or dendrogram thus constructed may reveal the underlying
taxonomy of themes in the data. It is a great tool for information
organization and data exploration.
...
Figure~\hyperref[fig-org-iris]{{[}fig-org-iris{]}}, we plot \emph{petal
length} vs.~\emph{petal width} and color code data in each class
(species).

There appear to be three clusters, as shown in
Figure~\hyperref[fig-org-iris]{{[}fig-org-iris{]}}, consistent with the
three species. But how can we design algorithms to identify these
clusters? Bear in mind that any clustering method, as unsupervised
learning, should be data driven and should aim to discover inherent
patterns from rather than imposing a structure on the data.
...
\item
  ...
  \tightlist
  \item
    zoomed in on the closest pair

    Step {1} of HAC clustering on IRIS data. The circle indicates a new
    cluster formed by the closest pair of instances. The centroid of the
    cluster is marked {×} in (b).
  \end{enumerate}
\end{enumerate}

Figure~\hyperref[fig-org-hac1]{1.1} (a) plots the IRIS data and
visualizes the \(1^{st}\) step of HAC clustering. Using Euclidean
distance, the closest pair of instances can be identified, as circled on
the plot, and joined as a new cluster.

Figure~\hyperref[fig-org-hac1]{1.1} (b) offers a closer look at the new
cluster. Once the new cluster is formed, it is represented by the
average values of its members, i.e.~the geometric center of the two data
instances on the plot (see \(\times\) in the circled cluster). Now this
new cluster with a centroid at \(\times\) is simply treated as another
data instance that can be compared to and merged with others.

Now we can continue to find the next closest pairs. In
Figure~\hyperref[fig-org-hac2]{1.2} (a), the \(1^{st}\) merged cluster
is found to be closest to yet another data instance, and they are joined
to form a new cluster again. This continues to merge with another
instance, as shown in Figure~\hyperref[fig-org-hac2]{1.2} (b).
...
\item
  ...
  \item
    ...
    Further iterations of HAC clustering on IRIS data
  \end{enumerate}
\end{enumerate}

Repeat the merging process, we can get results in
Figure~\hyperref[fig-org-hac3]{1.3}. If the desired number of clusters
\(k\) is predefined, one can stop the iterative process once \(k\)
clusters have been obtained. In Figure~\hyperref[fig-org-hac3]{1.3} (a),
for example, the clustering process stops at \(3\) clusters (\(k=3\)).
If one desires to build the entire hierarchy of the data, as
Figure~\hyperref[fig-org-hac3]{1.3} (b) shows, the process can continue
until all clusters have been merged into the root (\(k=1\)).
...
\item
  ...
  \item
    ...
    Result of HAC clustering on IRIS data
  \end{enumerate}
\end{enumerate}
...
\item
  ...
  \item
    ...
    K-means initial steps. A {×} represents an initial centroid.
    Undirected lines represent cluster membership. Directed lines {↗}
    indicates how the centroid will move (change) after re-computation
    based on assigned members.
  \end{enumerate}
\end{enumerate}

With data plotted in Figure~\hyperref[fig-org-kmeans1]{1.4} (a), we
start with \(3\) random cluster centroids, shown as \(\times\) in the
plot. For each data instance, we compute its distance to each of the
three initial centroids and identify the closest centroid, to which it
will be assigned as a member. Figure~\hyperref[fig-org-kmeans1]{1.4} (b)
shows lines connecting data instances to their corresponding closest
centroids.
...
(arrows) in Figure~\hyperref[fig-org-kmeans1]{1.4} (b) represent the
changes of cluster centroids.
...
\item
  ...
  \item
    ...
    K-means further steps. A {×} represents an centroid before it is
    updated. Undirected lines represent cluster membership. Directed
    lines {↗} indicates how the centroid will move (change) after
    re-computation based on assigned members.
  \end{enumerate}
\end{enumerate}
...
Figure~\hyperref[fig-org-kmeans2]{1.5} (a), which, in turn, leads to
further changes of cluster centroids. See the \(3^{rd}\) iteration in
Figure~\hyperref[fig-org-kmeans2]{1.5} (b). By repeating the iterations
of cluster member assignment and centroid re-computation, the cluster
centroids will continue to move and gradually converge. One may stop the
k-means optimization process when cluster centroid does not move or
moves very little, or after a certain number of iterations.
...
\item
  ...
  \tightlist
  \item
    Overview of centroid movement

    K-means final clusters and steps
  \end{enumerate}
\end{enumerate}
...
movement). Figure~\hyperref[fig-org-kmeans3]{1.6} (a) shows the final
\(3\) clusters thus identified and they are consistent with the three
species of Iris flowers in the data.
Figure~\hyperref[fig-org-kmeans3]{1.6} (b) gives an overview of the
movement of cluster centroids through the iterations. Even though we
started with random centroids, k-means gradually moves the centroids
toward more representative segments of the data and, in the end, reaches
an optimal outcome.
...
Figure~\hyperref[fig-org-kmeans3]{1.6} (a). This implies two basic
assumptions: 1) the distribution of member instances in a cluster is
spherical, and 2) each data instance only belongs to one and only one
cluster.
...
\(\delta_1=0.25\) for cluster \(c_3\).
Figure~\hyperref[fig-org-em1]{1.7} shows the distribution of IRIS data
instances on the petal width \(v\) dimension, with a normal (bell) curve
based on each set of initial parameters.

Initial parameters of normal distributions. {∘} are IRIS data instances.
{×} marks each an initial (random) mean {μ} (similar to a centroid),
around which a normal distribution curve is plotted. The shaded area
under curve shows the range of deviation from mean,
i.e.~{{[}μ − δ, μ + δ{]}}.
...
Figure~\hyperref[fig-org-em1]{1.7}). First, we compute \(P(v|c)\),
probability that each cluster model will generate the instance
\(v=0.2\):
...
As you can see from the Figure~\hyperref[fig-org-em1]{1.7}, \(v=0.2\) is
on the far left side of the first (red) bell curve centered on \(1\),
and has a very small probability density
\(P(v=0.2|c_1) \propto 0.00954\). This is further remove from the
centers of the second (blue) and third (green) normal curves, and
probability densities corresponding to \(c_2\) and \(c_3\) are much
smaller.
...
Figure~\hyperref[fig-org-em1]{1.7}.
...
First expectation result. Color of a data instance indicates the cluster
membership with the highest probability.

Figure~\hyperref[fig-org-em2]{1.8} shows the color-coded result of the
first \emph{Expectation} step on the IRIS data. A data instance's color
...
of data associated with them. In Figure~\hyperref[fig-org-em2]{1.8}, for
example, the first (red) bell curve is not centered on the data
instances most highly associated with it.
...
First Maximization result. A bell curve with with a shaded area
({±delta}) is the new model with updated parameters for the cluster.

Figure~\hyperref[fig-org-em3]{1.9} shows the result of this
\emph{Maximization} step. The models (normal curves) have moved closer
to their more highly associated data instances. This is very similar to
re-computation of centroids -- thus causing them to move -- in k-means
clustering. With Maximization, more than one parameters are needed to
specify a model. For normal distribution models, both mean \(\mu\) and
variance \(\delta^2\) are to be updated. If one assumes a different
probability distribution, then a different set of parameters will be
maximized.
...
Figure~\hyperref[fig-org-em4]{1.10} shows further iterations of EM on
the IRIS data. Visually, the cluster models (curves) move increasingly
closer to the data instances they represent and, in the end, get well
aligned with them.
...
Iterations of Expectation and Maximization. Note that there are {5} data
instances in each iris species. But because their petal width values
overlap on the only dimension, there appear to be {3} or {4} instances
in each cluster, which, in fact, has {5}.
...
\subsection{Term Weighting}\label{ch-text-weight}
...
\section{Similarity Measures}\label{ch-text-sim}
...
dimension, shown in Figure~\hyperref[ch-text-2d]{1.1}:
...
in question, as shown in Figure~\hyperref[ch-text-2d]{1.1}. In an
...
in Figure~\hyperref[ch-text-2d]{1.1}, their euclidean distance is:
...
Figure~\hyperref[ch-text-thought]{1.2}:

Thought experiment with two documents

For now, one may think the line distance in
Figure~\hyperref[ch-text-thought]{1.2} is a reasonable estimate of their
difference. But imagine make a double-copy of \(d_2\) and treat the new
copy as \(d_3\), resulting in Figure~\hyperref[ch-text-thought]{1.2}:

Thought experiment a third document, where {d3} doubles the content of
{d2}
...
However, as shown in Figure~\hyperref[ch-text-thought]{1.2}, \(d_3\) is
much farther away from \(d_1\) and consequently the euclidean distance
is much greater.
...
shown as dashed lines in Figure~\hyperref[ch-text-thought2]{1.3},
documents such as \(d_1\) and \(d_2\) can be treated as vectors from the
origin, with a magnitude (line length) and direction (arrow). The
difference between the two documents can be measured by their angle
distance.

Also in Figure~\hyperref[ch-text-thought2]{1.3}, \(d_3\) as copy of
\(d_2\) is pointing in the exact same direction of \(d_2\) and their
angle distance is \(0\). Consequently, the angle distance between
\(d_1\) and \(d_3\) is the same as that of \(d_1\) and \(d_2\).
...
that means, in the case of Figure~\hyperref[ch-text-thought2]{1.3}, to
what degree a document vector is more or less about \emph{machine} (to
the east) vs.~\emph{data} (to the north).
...
\protect\phantomsection\label{ch-text-cosine}{}

As shown in Figure~\hyperref[ch-text-cosine]{{[}ch-text-cosine{]}}, with
the help line perpendicular to vector \(d_1\), the cosine of \(d_1\) and
\(d_2\) is:
...
Figure~\hyperref[ch-text-cosine2]{{[}ch-text-cosine2{]}}, \(a\) is
almost the same as \(c\) and cosine similarity is nearly \(1\). The two
vectors are highly similar in their directions, with \(1\) indicating
the same direction and perfect match.

\protect\phantomsection\label{ch-text-cosine2}{}

On the other hand, when two vectors point to directions nearly
perpendicular to one another, with a large angle shown in
Figure~\hyperref[ch-text-cosine3]{{[}ch-text-cosine3{]}}, \(a\) becomes
extremely small and cosine similarity is close to \(0\). For vectors
based on text documents, their vector values are normally positive (in
the first quadrant in the two-dimensional space). Therefore, the
smallest cosine similarity value one can obtain is \(0\) when two
vectors are exactly perpendicular (orthogonal directions).

\protect\phantomsection\label{ch-text-cosine3}{}
...
relation can be visualized in Figure~\hyperref[fig-text-zipf]{1.4} (a).
...
\item
  ...
  \item
    ...
    Zipf law function, roughly for the top {1, 000} in English
  \end{enumerate}
\end{enumerate}

With a long tail, Figure~\hyperref[fig-text-zipf]{1.4} (a) on normal
coordinates is difficult to examine and it is often useful to apply the
logarithmic transformation. By taking the log of both \(x\) (rank) and
\(y\) (frequency) values, this can be transformed into
Figure~\hyperref[fig-text-zipf]{1.4} (b). As shown, Zipf law is a linear
function on log-log coordinates and is a special case of
\emph{power-law} distributions, which, as we discussed in Chapter~, are
results of the \emph{Matthew effect} (the rich get richer). In fact,
there is an intrinsic connection between the \emph{Principle of Least
Effort} and the \emph{Matthew effect} -- as we tend to a minimum set of
vocabulary, what we have frequently used (old "rich" words) will be used
even more frequently (richer).
...
As shown in Figure~\hyperref[fig-ir]{1.1}, the user with an information
need issues a query to the retrieval system, which in turn match it
against the document representation in the store to identify relevant
documents in the search result. The result can also be ranked (sorted)
in terms of predicted relevance of each document.

Basic paradigm of an {Information Retrieval} system
...
Figure~\hyperref[fig-ir]{1.1} and walk through some of the major ideas
for finding matched (and potentially relevant) documents.

In Figure~\hyperref[fig-ir]{1.1}, the data collection can be very large
with a great number of documents, e.g. a library of a million books. In
order to conduct searches with matching and ranking responsively, some
amount of data pre-processing is necessary. This is also practically
sensible if the collection is relatively static (no dramatic changes
frequently).
...
Two postings from the inverted index. Highlighted document IDs with
double border indicate matched documents that satisfy {Declaration} AND
{Independence}.
...
conclude that documents \(\#2\), \(\#103\), and \(\#456\) in Figure
~\hyperref[fig-ir-postings]{1.2} contain both terms. But how can an
...
shown in Figure~\hyperref[fig-ir-matching]{1.3}, where the
\emph{Declaration} posting has a value \(2\) and the \emph{Independence}
posting has \(1\) (see step 1 in the figure). As the one with a smaller
document ID, the \emph{Independence} posting will move to the next
document ID, which is \(2\) and matches the the current value in
\emph{Declaration} (see step 2).
...
Simultaneous walking to merge postings, starting at the first document
ID. The posting with the smaller value will move to the next ID.
...
Figure~\hyperref[fig-ir-matching]{1.3} step 3, then goes to document
\(\#3\) and switches the turn back as it surpasses \(2\). This process
will continue until no more matches can possibly be found.
...
reference one another via \emph{hyperlinks}.
Figure~\hyperref[fig-hlinks]{{[}fig-hlinks{]}} shows an example of a web
page, from which the user can click on a label (text), a button, or an
image to visit another page.

\begin{figure}[H]

{\centering \includegraphics[width=2in,height=\textheight,keepaspectratio]{chapters/../assets/figures/pageflow2.png}
...
In Figure~\hyperref[fig-hlinks]{{[}fig-hlinks{]}}, when page A links to
page B, descriptions or keywords page A uses on the hyperlink can be
considered for the representation of page B. Assuming the pages are on
different web sites with separate ownership, then this piece of anchor
text represents an independent evaluation of page B's contents. By
aggregating anchor text data from all other pages linking to page B, its
inverted index can be adjusted with the crowd-sourced data, which are
potentially more reliable than self-claimed page contents.\footnote{Anchor
  text can also be hacked and abused to boost one's search engine
  ranking. There has been known practice of self- or mutual-linking from
  sites set up by the same group of people. On the other hand, websites
  can team up to \emph{bomb} a target with off-topic anchor text in
  order to "promote" its ranking for those keywords, e.g.~for political
  activism.}
...
Figure~\hyperref[fig-ir-2links]{{[}fig-ir-2links{]}}, for example, page
\(t\) has two in-links from pages \(p_1\) and \(p_2\) and its impact
score is therefore \(2\).

\begin{figure}[H]

{\centering \includegraphics[width=1.5in,height=\textheight,keepaspectratio]{chapters/../assets/figures/ir/ir_2links.png}
...
Figure~\hyperref[fig-ir-2links]{{[}fig-ir-2links{]}}. Suppose \(p_1\) is
an ordinary page that is linked to by another page but links to many
others (including page \(d\)). On the other hand, \(p_2\) is a very
popular page that is referenced by many pages but has chosen to link to
only one page (i.e.~page \(d\)).
Figure~\hyperref[fig-ir-2links_more]{1.4} illustrates this scenario.
...
straightforward. Again in Figure~\hyperref[fig-ir-2links_more]{1.4}, the
impact of document \(d\) depends on the impact of linking pages \(p_1\)
and \(p_2\), which in turn depend on the ones that link to them. And
this will go on. How can we proceed to solve this problem of
\emph{chicken and egg} without going into a loop? But the solution is
indeed a loop, or a recursive process.
...
Figure~\hyperref[fig-eval-pr_curve]{{[}fig-eval-pr\_curve{]}} shows
precision-recall curves of two methods, where method 1 appears to
perform better as its curve is closer to the top-right (higher precision
and recall).
...
Figure~\hyperref[fig-eval-roc]{{[}fig-eval-roc{]}} illustrates a plot of
an ROC curve, where the diagonal line (dashed) defines the acceptable
performance. To understand what the diagonal line means, imagine we
create a random model that takes a 50-50 guess during classification. As
a result of this random guessing, there will be an equal split between
the predicted positives \(\hat{\oplus}\) and predicted negatives
\(\hat{\ominus}\). Hence, column values of the same row in
Table~\hyperref[tab-eval-cmatrix]{1.1} will be identical:
...
in Figure~\hyperref[fig-eval-roc]{{[}fig-eval-roc{]}} and is the result
of random guessing.
...
from their means. Figure~\hyperref[fig-eval-cor]{{[}fig-eval-cor{]}}
illustrates the connection using a 2-dimensional space. To simplify the
discussion, suppose there are only \(N=2\) instances on which the actual
outcomes are \((v_1, v_2)\) and predicted values are
\((\hat{v_1},\hat{v_2})\), with means \(\mu\) and \(\hat{\mu}\)
respectively.

As shown in Figure~\hyperref[fig-eval-cor]{{[}fig-eval-cor{]}}, the
actual values can be regarded as a vector pointing in the
\(\vec{v} = (v_1-\mu, v_2-\mu)\) direction, whereas the predicted is
